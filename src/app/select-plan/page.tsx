...
const PricingPlan: React.FC<
  PricingPlanProps & { isSelected: boolean; onSelect: () => void }
> = ({
  title,
  description,
  price,
...
export default function SelectPlanPage() {
  ...
  const handleGetAccess = (plan: Plan) => {
    ...
    if (isLoggedIn) {
      ...
      router.push(
        `${route}?plan=${encodeURIComponent(JSON.stringify(planData))}`
      );
    } else {
      router.push(route);
    }
  ...
  const handleNextStep = () => {
    const plan = PRICING_PLANS.find((p) => p.title === selectedPlan);
    if (plan) {
      handleGetAccess(plan);
    }
  };
